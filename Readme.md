...
This library solves the protocol bottleneck by packing 9-bit frames into continuous 72-bit hardware SPI chunks ($8\text{ pixels} = 9\text{ SPI bytes}$). Pixel blocks are streamed in 5.7 KB hardware bursts at up to 26 MHz, delivering full-screen refresh rates above 60 FPS without monopolizing system memory.
...
uint16_t width() const: Returns the current width in pixels (updates dynamically when rotated).

uint16_t height() const: Returns the current height in pixels (updates dynamically when rotated).
